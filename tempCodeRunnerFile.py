...
CELL_PHONE_CLASS_ID = 77   # COCO class ID
...
YAWN_TIME_WINDOW = 120          # 2 minutes
YAWN_RESET_TIME = 60            # 1 minute auto reset
...
eyes_closed_start_time = None   # <<< NEW (for blink filtering)
...
while True:
    ...
    if result.multi_face_landmarks:
        ...
        if gaze_vec[0] > 0.3:
            gaze_text = "Looking Right"     # NORMAL
        elif gaze_vec[0] < -0.3:
        ...
        if YAWN_DROWSY and yawn_drowsy_start_time is not None:
    ...
    # ---- PRIORITY ALERTS ----
    if EYES_CLOSED_DANGER:
        cv2.putText(frame,
                    "!!!! Danger Driver Eyes Closed",
                    (30, 210),
